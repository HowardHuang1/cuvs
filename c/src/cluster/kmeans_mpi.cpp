                                  ...
                                  int* n_iter)
{
  ...
  // Device selection: if this process sees only one GPU (CUDA_VISIBLE_DEVICES per process),
  // use device 0. Otherwise use rank so each MPI rank gets a different GPU.
  int device_count = 0;
  ...
  const T* X_ptr = static_cast<const T*>(X_host);
  rmm::device_uvector<T> X_local(static_cast<size_t>(n_local * n_cols), stream);
  raft::copy(X_local.data(), X_ptr + row_start * n_cols, n_local * n_cols, stream);
